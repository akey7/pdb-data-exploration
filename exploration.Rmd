...
```{r error=FALSE, warning=FALSE}
pdb <- read_csv("data/pdb_data_no_dups.csv", show_col_types = FALSE)
```
...
However, there are some plots where I don't restrict input data in this way. I will call them out below.
...
### Publications per macromolecule type

On partially cleaned data, with rows not having both a publicationYear and macromoleculeType being dropped.
...
```{r}
pdb %>%
  drop_na(publicationYear, macromoleculeType) %>%
  filter(publicationYear >= 2000, publicationYear < 2018) %>%
  group_by(macromoleculeType, publicationYear) %>%
...
- Mine the text of `classification`.
